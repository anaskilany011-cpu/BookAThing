...
# The movie used for booking MUST be the movie actually playing at this
# showtime — a movieId from a separate/unrelated /movies call won't match
# and the booking endpoint will (correctly) reject it. Pull it from the
# showtime's own embedded movie_id object instead of overwriting the
# earlier MOVIE_ID (still used for the review step later, where any valid
# movie is fine).
BOOKING_MOVIE_ID=$(echo "$RESP" | grep -o '"movie_id":{"_id":"[^"]*"' | head -1 | sed -E 's/.*"_id":"([^"]*)"/\1/')
echo "-> Movie actually playing at this showtime: $BOOKING_MOVIE_ID"
echo
...
echo "================================================"
RESP=$(curl -s -X POST "$BASE_URL/bookings" -H "Content-Type: application/json" -H "Authorization: Bearer $ACCESS_TOKEN" -d "{\"movieId\":\"$BOOKING_MOVIE_ID\",\"theaterId\":\"$THEATER_ID\",\"showtimeId\":\"$SHOWTIME_ID\",\"seats\":[\"A1\",\"A2\"],\"basePrice\":100}")
echo "$RESP"
